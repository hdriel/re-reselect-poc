...
const Title: React.FC<TitleProps> = ({totalProducts, updateStore}) => {
    ...
    return <div style={{display: 'flex', gap: '10px', justifyContent: 'space-between', alignItems: 'center'}}>
        <h1 style={{fontSize: '40px'}}>Product List ({totalProducts})</h1>
        <button onClick={() => updateStore()} style={{height: 'max-content', backgroundColor: '#0d011f', color: 'white'}}>ADD</button>
    </div>
}
